  ...
  <div>
    ...
    <template>
      ...
      <div style="margin: 10px;clear: both">
        ...
        <span style="margin-left: 100px">学号：</span>
        <el-input
            placeholder="请输入学号搜索"
            prefix-icon="el-icon-search"
            style="width: 10%"
            @change="searchByUserid"
            clearable
            v-model="userid">
        </el-input>
      </div>
...
export default {
  ...
  methods: {
    ...
    deleteUser(data,data1){
      ...
      if(confirm("是否确认删除？")){
        if(this.selectData.length === 0){
          let list = []
          list.push({id:id})
          _this.$axios.post('/user/delete',list,token).then(res=>{
    ...
    searchByName(data){
      const _this = this
      _this.$axios.get('/user/searchbyname?username='+data+'&currentPage='+_this.currentPage,{
        headers: {
          Authorization: localStorage.getItem('token')
        }
      }).then(res=>{
